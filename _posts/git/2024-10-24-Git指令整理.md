...
#### 其他
    ...
    # 本地拉某一个pr代码的另一种方式
    $ git ls-remote upstream  | grep 790
    $ git pull upstream refs/merge-requests/790/head

	# git add 添加了多余文件，撤销操作
    $ git reset HEAD [file]

    # 切换分支并且与远端分支关联
    $ git fetch upstream  - - - 先把远端所有的分支拉到本地
    $ git checkout -t upstream/release-2.0

### 删除本地和本地远端除了master 的所有分支的脚本
```shell
#!/usr/bin/env bash
